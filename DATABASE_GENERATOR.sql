...
CREATE TABLE IF NOT EXISTS `zil3-zpilvenma`.`POINT_RETRAIT` (
  `POINTRETRAIT_id` INT NOT NULL AUTO_INCREMENT,
  `POINTRETRAIT_nom` VARCHAR(45) NOT NULL,
  `POINTRETRAIT_adresse` VARCHAR(100) NOT NULL,
  PRIMARY KEY (`POINTRETRAIT_id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `zil3-zpilvenma`.`VENDEUR` (
  ...
  `VENDEUR_mot_de_passe` CHAR(64) NOT NULL,
  `POINTRETRAIT_id` INT NOT NULL,
  INDEX `fk_VENDEUR_POINT_RETRAIT1_idx` (`POINTRETRAIT_id` ASC),
  PRIMARY KEY (`VENDEUR_id`),
  UNIQUE INDEX `VENDEUR_login_UNIQUE` (`VENDEUR_login` ASC),
  CONSTRAINT `fk_VENDEUR_POINT_RETRAIT1`
    FOREIGN KEY (`POINTRETRAIT_id`)
    REFERENCES `zil3-zpilvenma`.`POINT_RETRAIT` (`POINTRETRAIT_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `zil3-zpilvenma`.`LATABLEQUICONTIENTLESADMINISTRATEURSCOOLS` (
  ...
  `LTQCLAC_mot_de_passe` CHAR(64) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `zil3-zpilvenma`.`COMMANDE` (
  ...
  `COMMANDE_code` CHAR(60) NOT NULL,
